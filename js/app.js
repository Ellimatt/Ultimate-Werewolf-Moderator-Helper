...
function updateBackButton() {
    ...
    const continueButton = [...screen.querySelectorAll("button")].find(button =>
    ...
    if (
        ...
        existingButtons[0].nextElementSibling === continueButton
    ) {
        return;
    }
    ...
    continueButton.insertAdjacentElement("beforebegin", backButton);
...
function saveGameState() {
    ...
    if (
        lastSavedGameState &&
        lastSavedGameState.screenHTML !== state.screenHTML
    ) {
        backSavedGameState = lastSavedGameState;
    }
...
function startGamePersistence() {
    ...
    const screen = document.getElementById("screen");
    const observer = new MutationObserver(scheduleGameSave);
...
function removeAllPlayers() {
    ...
    if (!players.length || !confirm("Are you sure you want to remove all players?")) {
        return;
    }
...
function removeAllRoles() {
    ...
    if (!roles.some(role => Number(role.count) > 0) ||
        !confirm("Are you sure you want to set every role card to zero?")) {
        return;
    }
...
function drawGameResult(message, continuation = null) {
