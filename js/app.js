...
const roleHelpText = {
    ...
    "Prince": "When voted for elmination their role is revealed and they stay in the game",
    ...
    ,"Bailiff": "Secretly breaks a tied daytime elimination vote according to the Bailiff's vote."
    ...
    ,"Witch Doctor": "Gives one player a one-night voodoo doll that redirects a Werewolf attack to the closest Werewolf."
    ,"Night Watch": "Links two players as a patrol and learns whether either specifically targeted the other that night."
    ,"Watchman": "Near the end of each night, chooses a player and learns how many night roles visited that player."
    ,"Pack's Bond": "When Pack's Bond is the last elimination-capable Werewolf, the Werewolves eliminate two players each night."
    ,"Pack's Hunger": "If the Werewolves complete two consecutive nights without eliminating anyone, they eliminate two players on the third night."
    ,"Blood Wolf": "While the Blood Wolf is alive on Night 3, Werewolf attacks ignore every protection, Tough Guy's delay, and the Diseased penalty."
    ,"Big Bad Wolf": "Counts as two Werewolves when checking Werewolf parity."
    ,"Bloodscent": "After the Werewolves eliminate a player, learns that player's exact role on the following night."
    ,"Nightstalker": "Wakes with the Werewolves and also inspects one player's exact role each night."
};
...
function showGameGuide() {
    ...
    overlay.innerHTML = `<div class="appModal infoModal gameGuideModal" role="dialog" aria-modal="true" aria-labelledby="gameGuideTitle">
        ...
        <ul class="guideList">
            <li><strong>Exact-role checks</strong>, such as Mystic Seer or Nightstalker, need exact identities for every possible target.</li>
        ...
        <dl class="guideDefinitions">
            ...
            <dt>Elimination-capable Werewolf</dt><dd>A Werewolf-team role that can participate in the shared Werewolf elimination. Minion and Sorceress cannot perform that elimination. If no elimination-capable Werewolf remains, the Villagers win.</dd>
...
function startNightOne() {
    ...
    const anotherEliminationWolfSelected = roles.some(role =>
        Number(role.count) - (leftoverCardRole?.role === role.role ? 1 : 0) > 0 &&
        role.team === "Werewolf" &&
...
function drawNightRole() {
    ...
    if (role.alphaActionOnly || role.sharedWolfEliminationOnly) {
...
function remainingNightOneRoleCount(roleDefinition) {
    const dealtCount = Math.max(
        0,
        Number(roleDefinition?.count || 0) - (leftoverCardRole?.role === roleDefinition?.role ? 1 : 0)
    );
...
function nightOneActionHasRequiredInformation(role) {
    ...
    const exactRoleCheck = role.action === "RevealTeam" ||
        (role.action === "RevealRole" && role.role === "Nightstalker");
    if (exactRoleCheck) {
...
function startNightOneActions(role) {
    ...
    players
        .filter(player =>
            (role.sharedWolfEliminationOnly ? canChooseWerewolfElimination(player) : player.role === role.role) &&
            player.alive &&
        ...
        .forEach(actor => {
            nightOneActionOrder.push({
                role,
                actor,
                continuesIdentificationWake: !isLaterNight && !role.alphaActionOnly && !role.sharedWolfEliminationOnly
            });
        });
...
function drawNightAction() {
    ...
    const actorMayTargetSelf = item.role.action === "ProtectPlayer" ||
        ...
        (item.role.action === "SilencePlayer" && item.role.role === "Black Wolf" && roleRuleSettings.blackWolfMaySilenceSelf) ||
    ...
    const roleRepeatRule = {
        ...
        "Black Wolf": ["blackWolfMayRepeatTarget", "lastBlackWolfTargetId"],
...
function confirmNightAction() {
    ...
    const roleLastTargetKey = {
        ...
        "Black Wolf": "lastBlackWolfTargetId",
...
function drawNightActionResult(item, targets, includeQuestion = false) {
    ...
    const isNightstalkerCheck = item.actor.role === "Nightstalker" && item.role.action === "RevealRole";
...
function getNightActionResult(item, targets) {
    ...
    switch (item.role.action) {
        ...
        case "RevealRole":
            return item.actor.role === "Nightstalker" ? `${target.name} — ${target.role}` : `${target.name} was checked.`;
...
function refreshWolfPassiveBonusesForCurrentNight() {
    ...
    const packBondDoubleKill = livingEliminationWolves.length === 1 && livingEliminationWolves[0].role === "Pack's Bond";
    const packHungerDoubleKill = currentNight >= 3 &&
        players.some(player => player.alive && player.role === "Pack's Hunger") &&
    ...
    bloodWolfBypassesProtectionTonight = currentNight === 3 &&
        players.some(player => player.alive && player.role === "Blood Wolf");

    if (packBondDoubleKill) {
        recordPhaseEvent(`Night ${currentNight}`, "Pack's Bond was the last elimination-capable Werewolf, so the Werewolves received two eliminations.", `pack-bond-double-${currentNight}`);
    }
    if (packHungerDoubleKill) {
        recordPhaseEvent(`Night ${currentNight}`, "The Werewolves had gone two consecutive nights without an elimination, so Pack's Hunger granted two eliminations.", `pack-hunger-${currentNight}`);
    }
    if (bloodWolfBypassesProtectionTonight) {
        recordPhaseEvent(`Night ${currentNight}`, "The living Blood Wolf allowed Werewolf attacks to ignore all protection, Tough Guy's delay, and the Diseased penalty on Night 3.", `blood-wolf-${currentNight}`);
    }
}
...
function startNextNight() {
    ...
    players.filter(player => player.alive && player.role === "Bloodscent" && bloodscentVictims.length).forEach(actor => {
        nightOneActionOrder.push({
            role: { role: "Bloodscent", priority: 996.5, question: "Learn the exact roles of players eliminated by the Werewolves last night.", action: "RevealBloodscentVictims" },
            actor
        });
    });
    const hiddenDeadBloodscent = players.find(player =>
        !player.alive && !player.roleRevealed && player.role === "Bloodscent"
    );
...
function getNightModeratorDetails() {
    ...
    players.filter(player => player.bloodWolfAttackTonight).forEach(player => {
        details.push(`${player.name} was targeted during the Blood Wolf's Night 3 attack, so protection did not apply.`);
    });
...
function drawDayOne() {
    ...
    const eliminationRows = eliminated.map(player =>
        `<p>${readAloud(`Eliminated: ${player.name}${player.roleRevealed ? ` — ${revealedRoleName(player)}` : ""} — ${player.deathCause || player.pendingDeathCause || "Night attack"}.`)}</p>`
    ).join("");
...
function drawVoodooRetaliation(protectedPlayer) {
    ...
    document.getElementById("screen").innerHTML = `
        <h2>☀️ Day ${currentDay}</h2>
        <h3>Voodoo Doll</h3>
        <div class="moderatorPanel">${escapeHTML(protectedPlayer.name)} had the voodoo doll and was attacked by the Werewolves. ${roleRuleSettings.voodooDollTargetSurvives ? "They survive." : "They are still eliminated."} Select the elimination-capable Werewolf seated physically closest to them.</div>
        <select id="voodooWolfTarget"><option value="">Select closest Werewolf</option>${wolves.map(player => `<option value="${players.indexOf(player)}">${escapeHTML(player.name)}</option>`).join("")}</select>
...
function resolveVoodooRetaliation(protectedIndex) {
    ...
    if (!protectedPlayer || !selected || selected.value === "" || !wolf?.alive || !canChooseWerewolfElimination(wolf)) {
        alert("Select the closest elimination-capable Werewolf.");
        return;
    }
    ...
    recordPhaseEvent(
        `Night ${currentNight}`,
        `${protectedPlayer.name}'s voodoo doll eliminated ${wolf.name}.${roleRuleSettings.voodooDollTargetSurvives ? ` ${protectedPlayer.name} survived the attack.` : ` ${protectedPlayer.name} was also eliminated by the attack.`}`,
...
function drawDayOnePlayers() {
    ...
    [...activePlayers, ...unavailablePlayers].forEach(player => {
        ...
        const displayedRole = !player.alive ?
            `${player.roleRevealed ? "Role revealed" : "Role not revealed"} - ${player.roleRevealed ? revealedRoleName(player) : (player.role || "Unknown")}` :
            (player.role || "Unknown");
...
function startVote(index) {
    ...
    const votingReminders = [
        ...
        bailiff ? `${bailiff.name} is the Bailiff; if the vote ties, resolve it according to the Bailiff's secret vote.` : "",
...
function completeVoteElimination(index) {
...
function resolveMartyrDecision(martyrIndex, eliminatedIndex, inheritRole) {
    ...
    if (martyr?.alive && eliminatedPlayer && inheritRole) {
        ...
        const inheritedTeam = eliminatedPlayer.team;
        inheritRoleState(martyr, eliminatedPlayer);
...
function drawGameResult(message, continuation = null, reviewOnly = false) {
    ...
    const eliminatedRosterRows = deadPlayers.length ? deadPlayers.map(player => `
        <li><strong>${escapeHTML(player.name)}</strong> — ${escapeHTML(revealedRoleName(player))}</li>
    `).join("") : "<li>None</li>";
    ...
    const deadRows = deadGroups.length ? deadGroups.map(group => `
        ...
        <ul>
            ${group.players.map(player => `
                <li>
                    <strong>${escapeHTML(player.name)}</strong> —
                    ${escapeHTML(revealedRoleName(player))} —
...
function buildGameSummaryText() {
    ...
    const lines = [
        ...
        ...(deadPlayers.length ? deadPlayers.map(player =>
            `${player.name} - ${revealedRoleName(player)}`
        ) : ["None"]),
        "",
        "Full Game Timeline"
    ];
