...
const defaultRoleRuleSettings = {
    ...
    madBomberActivatesDuringDay: true,
    madBomberActivatesDuringNight: true,
    sorceressRevealsWerewolf: false,
    wildChildTransformsOnRoleModelDeath: true,
    ...
    beholderLearnsApprenticeSeer: true,
    beholderLearnsDoppelgangerSeer: true,
...
const roleHelpText = {
    ...
    "Beholder": "Wakes on Night 1 and is privately shown who the Seer is.",
    ...
    "Doppelganger": "Chooses a player on Night 1 and is shown their current role each night. If that player dies, the Doppelganger inherits their role and team.",
    ...
    "Mad Bomber": "When eliminated, the two living players currently seated beside the Mad Bomber are eliminated.",
    "Magistrate": "Chooses one player each night who may not be eliminated by the village vote the following day.",
    "Martyr": "After a successful Day vote and before the eliminated role is revealed, may inherit that player's role and team.",
    "Mason": "A Villager who wakes on Night 1 to identify the other Masons.",
    "Mayor": "A revealed Mayor's vote counts twice while at least one other Villager-team player remains.",
    ...
    "P.I.": "Inspects a player and that player's current neighbors for Werewolf-team presence. The inspected player cannot also be entered as a neighbor.",
    "Pacifist": "A Villager whose role affects how the moderator handles village voting.",
    ...
    "Sorceress": "Looks for the Seer and wins with the Werewolf team, but cannot make the shared Werewolf elimination.",
    ...
    "Tracker": "If a neighboring player was eliminated by the Werewolves, may inspect one player to learn whether they appear to be a Werewolf.",
    "Village Idiot": "A Villager whose role affects how the moderator handles village voting.",
    ...
    "Fruit Brute": "Counts toward the Werewolf team for victory, but cannot participate in the shared Werewolf elimination.",
    "Wild Child": "Chooses a role model on Night 1 and becomes a Werewolf when that player dies."
};
...
function willBeEliminatedByWolvesTonight(player) {
    return !!player?.wolfAttackCountsTonight &&
        !player.pendingAlphaConversion &&
        !["Cursed", "Tough Guy", "Butcher"].includes(player.role) &&
        !(player.role === "Shepherd" && player.shepherdFlockAlive);
}
...
function madBomberShouldActivate(player) {
    return roleDeathWasAtNight(player) ?
        ...
        roleRuleSettings.madBomberActivatesDuringDay;
}
...
function roleOptionMarkup(roleName) {
    ...
    if (roleName === "Mad Bomber") {
        options.push(["madBomberActivatesDuringDay", "Activates when eliminated during the Day"]);
    ...
    if (roleName === "Wild Child") {
        options.push(["wildChildTransformsOnRoleModelDeath", "Transforms when the role model dies"]);
        options.push(["wildChildTransformsAfterDayDeath", "Transforms after a Day death"]);
        options.push(["wildChildTransformsAfterNightDeath", "Transforms after a Night death"]);
        options.push(["wildChildTransformsAfterKick", "Transforms when the role model is Kicked"]);
        options.push(["wildChildTransformsIfRoleHidden", "Transforms when the role model's role remains hidden"]);
    }
    ...
    if (roleName === "Tracker") {
        options.push(["trackerCountsLycanAsWolf", "Lycan appears as a Werewolf to the Tracker"]);
        options.push(["trackerHidesWolfMan", "Wolf Man appears as a Villager to the Tracker"]);
    }
...
function confirmNightAction() {
    ...
    if (targetCount === 1) {
        ...
        if (["Investigate", "TrackWolf"].includes(item.role.action)) {
            ...
            if (item.role.action === "Investigate" && item.neighbors.includes(targets[0])) {
                alert("The inspected player cannot also be selected as a neighbor.");
                return;
            }
            if (
                item.role.action === "TrackWolf" &&
                !item.neighbors.some(player => willBeEliminatedByWolvesTonight(player))
            ) {
                alert("The Tracker may inspect only when a current neighbor was eliminated by the Werewolves tonight. Choose Nobody instead.");
                return;
            }
...
function drawNightActionResult(item, targets, includeQuestion = false) {
    ...
    const signalText = item.role.action === "RevealAlignment" ?
        ...
        item.role.action === "TrackWolf" ?
            readAloud(signalIsUp ? "Thumbs up: Werewolf." : "Thumbs down: not a Werewolf.") :
...
function updateWildChildren() {
    players.filter(player =>
        ...
        roleRuleSettings.wildChildTransformsOnRoleModelDeath &&
