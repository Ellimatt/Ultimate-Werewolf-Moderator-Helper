...

{
    ...
    context.players = [player("Crimson Wolf", "Werewolf"), target];
    context.wolvesDisabledTonight = true;
...
{
    const phantom = player("Phantom Wolf", "Werewolf");
    const target = player("Villager");
    context.executeAction("MarkLycan", phantom, [target]);
    assert.equal(target.appearsAsWerewolfOverride, true);
    assert.equal(target.phantomMarked, true);
}

{
    const pet = player("Pet Wolf", "Werewolf");
    const owner = player("Villager");
    context.executeAction("ChooseOwner", pet, [owner]);
    assert.equal(pet.petOwner, owner);
}
...

{
    ...
    const ward = player("Villager");
    context.executeAction("ChooseGuardianWard", guardian, [ward]);
    assert.equal(guardian.guardianWard, ward);
    assert.equal(ward.guardian, guardian);
}

{
    const exposer = player("Exposer");
    const target = player("Villager");
    context.executeAction("ExposeRole", exposer, [target]);
    assert.equal(exposer.exposedPlayerTonight, target);
    assert.equal(target.exposedByTonight, exposer);
}

{
    context.playerAppearsAs = target => target.team;
    const oracle = player("Oracle");
    const wolf = player("Werewolf", "Werewolf");
    context.executeAction("OracleInspect", oracle, [wolf]);
    assert.equal(oracle.oracleUnresolvedWolf, wolf);
    const villager = player("Villager");
    context.executeAction("OracleInspect", oracle, [villager]);
    assert.equal(oracle.oracleUnresolvedWolf, wolf);
    assert.equal(oracle.oracleBlockedSelection, villager);
}

{
    const silversmith = player("Silversmith");
    context.executeAction("KillPlayer", player("Werewolf", "Werewolf"), [silversmith]);
    assert.equal(silversmith.silversmithBlockedWerewolfTonight, true);
    assert.notEqual(silversmith.attackedTonight, true);
}
