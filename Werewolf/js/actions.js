    ...
    switch (actionName) {
        ...
        case "CopyRole": {
            ...
            if (!copiedPlayer.alive && !actor.doppelgangerInheritedRole) {
                const inheritedDefinition = roles.find(role => role.role === copiedPlayer.role);
                const receivesFreshAbility = typeof roleRuleSettings === "undefined" ||
                    roleRuleSettings.doppelgangerReceivesFreshOncePerGameAbility !== false;
                actor.role = copiedPlayer.role;
                actor.team = copiedPlayer.team;
                actor.doppelgangerInheritedRole = copiedPlayer.role;
                actor.doppelgangerTransformedTonight = true;
                actor.usedOncePerGameAction = inheritedDefinition?.oncePerGame ? !receivesFreshAbility : false;
                actor.princeVoteSurvived = false;
                actor.toughGuyDeathPending = false;
                actor.shepherdFlockAlive = false;
                actor.roleModel = null;
                actor.butcherRedirectTarget = null;
            }
