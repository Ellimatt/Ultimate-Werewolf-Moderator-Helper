...
function executeAction(
    ...
    switch (actionName) {
        ...
        case "KillPlayer":
            ...
            if (!bloodWolfBypassesProtectionTonight && targets[0].role === "Silversmith") {
                targets[0].silversmithBlockedWerewolfTonight = true;
                return true;
            }
        ...
        case "MarkLycan":
            ...
            targets[0].phantomMarked = true;
            targets[0].appearsAsWerewolfOverride = true;
            return true;

        case "ChooseOwner":
            if (!targets.length) return false;
            actor.petOwner = targets[0];
            return true;

        case "ChooseGuardianWard":
            if (!targets.length) return false;
            actor.guardianWard = targets[0];
            targets[0].guardian = actor;
            return true;

        case "ExposeRole":
            if (!targets.length) return false;
            actor.exposedPlayerTonight = targets[0];
            targets[0].exposedByTonight = actor;
            return true;

        case "OracleInspect":
            if (!targets.length) return false;
            actor.oracleLastTarget = targets[0];
            if (actor.oracleUnresolvedWolf?.alive) {
                actor.oracleBlockedSelection = targets[0];
                return true;
            }
            if (playerAppearsAs(targets[0]) === "Werewolf") {
                actor.oracleUnresolvedWolf = targets[0];
            }
            return true;
        ...
        case "CopyRole": {
            ...
            // Repair games created before Martyr handoffs retained the target's
            // original role. A Doppelganger must wait for the Martyr holding
            // that role, rather than permanently inheriting the public Martyr
            // card placed on the eliminated target.
            if (
                actor.doppelgangerInheritedRole === "Martyr" &&
                copiedPlayer.roleBeforeMartyrExchange
            ) {
                actor.role = "Doppelganger";
                actor.team = "Villager";
                actor.doppelgangerInheritedRole = null;
                actor.doppelgangerTransformedTonight = false;
                actor.doppelgangerDeathRoleCanTriggerAgain = false;
            }
            ...
            if (!copiedPlayer.alive && !actor.doppelgangerInheritedRole) {
                ...
                const inheritedTeam = copiedPlayer.teamBeforeMartyrExchange ||
                    (typeof roleTeamForInheritance === "function" ? roleTeamForInheritance(copiedPlayer) : copiedPlayer.team);
                const martyrHolder = players.find(player =>
                    (
                        copiedPlayer.roleInheritedByMartyrId != null &&
                        player.id === copiedPlayer.roleInheritedByMartyrId
                    ) ||
                    (
                        !copiedPlayer.roleInheritedByMartyrId &&
                        player.martyrUsed &&
                        player.role === inheritedRole
                    )
                );
                if (martyrHolder?.alive) {
                    return true;
                }
                if (
                    inheritedRole === "Seer" &&
        ...
        case "BecomeSeer": {
            ...
            if (actor.becameSeerTonight) {
                ...
                actor.role = "Seer";
                actor.team = actor.isThing ? "Werewolf" : "Villager";
            }
...
function inheritRoleState(inheritor, source) {
    ...
    const retainsThingTeam = inheritor.isThing === true;
    inheritor.role = source.role;
    inheritor.team = source.team;
    if (retainsThingTeam) inheritor.team = "Werewolf";
