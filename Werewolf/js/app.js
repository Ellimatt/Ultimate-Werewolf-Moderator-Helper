...
const defaultGameSettings = {
    ...
    ravenousWolfCountsNightZero: false
};
...
function showGameSettings() {
    ...
    overlay.innerHTML = `<div class="appModal infoModal" role="dialog" aria-modal="true" aria-labelledby="gameSettingsTitle">
        ...
        ${!gameSettings.werewolfEliminationOnFirstNight ? `<div style="margin-left:1.25rem;">${settingToggle("ravenousWolfCountsNightZero", "Begin the Ravenous Wolf no-elimination tally on Night 0", "Counts the setup night toward Ravenous Wolf's consecutive nights without an elimination.")}</div>` : ""}
...
function drawRoleScreen() {
    ...
    let html = `
        ...
        <button type="button" onclick="scrollRolePage('bottom')">↓ Scroll to Bottom</button>
    ...
    html += `
        ...
        <button type="button" onclick="scrollRolePage('top')">
            ↑ Scroll to Top
        </button>
...
function refreshWolfPassiveBonusesForCurrentNight() {
    ...
    const ravenousCountsNightZero = !gameSettings.werewolfEliminationOnFirstNight && gameSettings.ravenousWolfCountsNightZero;
    const ravenousTallyCanTrigger = currentNight >= (ravenousCountsNightZero ? 2 : 3);
    const packHungerDoubleKill = ravenousTallyCanTrigger &&
        players.some(player => player.alive && player.role === "Ravenous Wolf") &&
        [currentNight - 1, currentNight - 2].every(night => night === 0 || !werewolfEliminationOccurredOnNight(night));
