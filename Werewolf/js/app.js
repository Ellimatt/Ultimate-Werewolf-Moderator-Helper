...
function showGameSettings() {
    ...
    overlay.innerHTML = `<div class="appModal infoModal" role="dialog" aria-modal="true" aria-labelledby="gameSettingsTitle">
        ...
        <label class="ruleToggle">
            <input type="checkbox" ${gameSettings.werewolfEliminationOnFirstNight ? "checked" : ""} onchange="setGameSetting('werewolfEliminationOnFirstNight', this.checked); showGameSettings()">
...
function drawRoleScreen() {
    ...
    let html = `
        ...
        <div class="rolePageActions rolePageActionsTop">
            <button class="primaryAction startGameTop" type="button" onclick="drawGameConfirmation()" ${cards !== requiredCards ? "disabled" : ""}>Start Game</button>
            <button type="button" onclick="promptToSaveGameSetup()" ${cards === 0 ? "disabled" : ""}>Save Loadout</button>
            <button class="quietAction" type="button" onclick="scrollRolePage('bottom')">↓ Bottom</button>
        </div>
    ...
    html += `
        ...
        <div class="rolePageFooter">
            <button class="dangerButton removeRolesButton" type="button" onclick="removeAllRoles()">Remove All Roles</button>
            <div class="rolePageActions">
                <button class="quietAction" type="button" onclick="drawPlayerScreen()">⬅ Back</button>
                <button class="quietAction" type="button" onclick="scrollRolePage('top')">↑ Top</button>
                <button class="primaryAction" type="button" onclick="drawGameConfirmation()" ${cards !== requiredCards ? "disabled" : ""}>Start Game</button>
            </div>
        </div>
...
function showGameStateCorrection() {
    ...
    overlay.innerHTML = `<div class="appModal infoModal correctionModal" role="dialog" aria-modal="true" aria-labelledby="correctionTitle">
        ...
        <label class="correctionPlayerPicker">Player<select id="correctionPlayer" onchange="loadCorrectionPlayer()">
        ...
        <fieldset class="correctionGroup"><legend>Identity</legend>
            <div class="correctionGrid">
                <label>Current role<select id="correctionRole" onchange="updateCorrectionRoleTeam(); updateCorrectionPreview()">${roleOptions.map(role => `<option value="${escapeHTML(role)}">${escapeHTML(role)}</option>`).join("")}</select></label>
                <label>Current team<select id="correctionTeam" onchange="updateCorrectionPreview()"><option value="Villager">Villager</option><option value="Werewolf">Werewolf</option></select></label>
            </div>
            <label class="ruleToggle"><input id="correctionRevealed" type="checkbox" onchange="updateCorrectionPreview()"><span><strong>Role is publicly revealed</strong><small>Controls what the village knows</small></span></label>
        </fieldset>

        <fieldset class="correctionGroup"><legend>Life status</legend>
            <label>Status<select id="correctionStatus" onchange="updateCorrectionDeathFields(); updateCorrectionPreview()"><option value="alive">Alive</option><option value="eliminated">Eliminated</option></select></label>
            <div id="correctionDeathFields" class="correctionGrid">
                <label>Cause of death<input id="correctionCause" type="text" list="correctionCauses" placeholder="Werewolf attack" oninput="updateCorrectionPreview()"></label>
                ...
                <label>Phase<select id="correctionPhase" onchange="updateCorrectionPreview()">${phaseOptions.map(phase => `<option value="${phase}">${phase}</option>`).join("")}</select></label>
            </div>
        </fieldset>

        <fieldset class="correctionGroup"><legend>Persistent effects</legend>
            <div class="correctionGrid">
                <label>Armor charges<input id="correctionArmor" type="number" min="0" max="9" value="0" oninput="updateCorrectionPreview()"></label>
                <label>If this player dies, I die<select id="correctionConnectedTo" onchange="updateCorrectionPreview()"><option value="">Nobody</option>${players.map((player, index) => `<option value="${index}">${escapeHTML(player.name)}</option>`).join("")}</select></label>
            </div>
            <label class="ruleToggle"><input id="correctionAbilityUsed" type="checkbox" onchange="this.nextElementSibling.querySelector('small').textContent = this.checked ? 'Used' : 'Unused'; updateCorrectionPreview()"><span><strong>One-use ability status</strong><small>Unused</small></span></label>
        </fieldset>
