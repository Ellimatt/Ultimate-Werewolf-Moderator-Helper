...
const roleHelpText = {
    ...
    "Black Wolf": "A Werewolf who also silences one player each night.",
    ...
    ,"Pack's Bond": "When Pack's Bond is the last Werewolf, the Werewolves eliminate two players each night."
...
function summaryRoleName(player) {
    ...
    if (player.alive && player.martyrUsed) return `(Martyr): ${player.role || "Unknown"}`;
    if (player.alive && player.isDoppelganger && player.doppelgangerInheritedRole) {
    ...
    players
        ...
        .forEach(actor => {
            nightOneActionOrder.push({
                role,
                actor,
                continuesIdentificationWake: !isLaterNight && role.action !== "KillPlayer" && !role.alphaActionOnly && !role.sharedWolfEliminationOnly
            });
        });
...
function refreshWolfPassiveBonusesForCurrentNight() {
    ...
    if (packBondDoubleKill) {
        recordPhaseEvent(`Night ${currentNight}`, "Pack's Bond was the last Werewolf, so the Werewolves received two eliminations.", `pack-bond-double-${currentNight}`);
    }
...
function endDayWithoutElimination(recordNoVote = true) {
    ...
    document.getElementById("screen").innerHTML = `
        <h2>☀️ Day ${currentDay}</h2>
        <h3>${readAloud(`${transformed.map(player => player.name).join(" and ")} ${transformed.length === 1 ? "was" : "were"} the Sasquatch and ${transformed.length === 1 ? "has" : "have"} become ${transformed.length === 1 ? "a Werewolf" : "Werewolves"}.`)}</h3>
...
function drawGameResult(message, continuation = null, reviewOnly = false) {
    ...
    const finalEliminationRows = newlyEliminatedPlayers.map(player => `
        <li>${readAloud(`${player.name} was eliminated by ${player.deathCause || "an unknown cause"}.`)}</li>
    `).join("");
